...
def parse_witness(form_data, db_service, person_repo, event_idx: int, witness_idx: int):
    ...
    sex = parse_sex(sex_str)
    kind_str = (
        get_first(form_data, f"e{event_idx}_witn{witness_idx}_kind", "witness")
        .strip()
        .upper()
    )
    kind_map = {
...
def implem_route_ADD_FAM(base, lang="en"):
    ...
    if request.method == "POST":
        ...
        event_indices = set()
        for key in form_data.keys():
            if key.startswith("e_name") or (
                key.startswith("e") and "_" in key and not key.startswith("e_")
            ):
                try:
        ...
        if (
            family_events
            and hasattr(family_events[0].name, "__class__")
        ...
        lib_family = LibFamily[int, int, str](
            index=None,
